...
const TextLayerContent = memo(({ layer, isEditing, onCommit }: { layer: Layer; isEditing: boolean; onCommit: (v: string) => void }) => {
...
const ShapeLayerContent = memo(({ layer }: { layer: Layer }) => {
  ...
  if (layer.shape === "circle") {
    return <div style={{ width: "100%", height: "100%", borderRadius: "50%", background: fillStyle, border: layer.strokeWidth ? `${layer.strokeWidth}px solid ${layer.stroke || "#fff"}` : undefined }} />;
  }
  if (layer.shape === "triangle") {
    return <div style={{ width: 0, height: 0, borderLeft: `${layer.width / 2}px solid transparent`, borderRight: `${layer.width / 2}px solid transparent`, borderBottom: `${layer.height}px solid ${layer.fill || "#00d4ff"}` }} />;
  }
  return <div style={{ width: "100%", height: "100%", background: fillStyle, border: layer.strokeWidth ? `${layer.strokeWidth}px solid ${layer.stroke || "#fff"}` : undefined, borderRadius: layer.borderRadius ? `${layer.borderRadius}px` : undefined }} />;
});
...
const LayerRenderer = memo(({ layer, isSelected, isPlaying, isEditingText, onMouseDown, onResizeStart, onDoubleClick, onContextMenu }: {
  layer: Layer;
  isSelected: boolean;
  isPlaying: boolean;
  ...
  return (
    <div
      style={style}
      className={`canvas-element ${isSelected ? "selected" : ""} ${isEditingText ? "editing-text" : ""}`}
    ...
    >
      {layer.type === "text" && (
        <TextLayerContent
          ...
          onCommit={v => updateLayer(layer.id, { content: v })}
        />
      )}
      {layer.type === "shape" && <ShapeLayerContent layer={layer} />}
      {layer.type === "image" && layer.src && (
        <img src={layer.src} alt={layer.name} style={{ width: "100%", height: "100%", objectFit: layer.objectFit || "contain", display: "block" }} draggable={false} />
      )}
      {layer.type === "icon" && (
        <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", color: layer.color || "#00d4ff", fontSize: layer.fontSize || 48, userSelect: "none", textShadow: layer.textShadow || undefined }}>
          {layer.content || "★"}
        </div>
      )}
      {layer.type === "html" && (
        <div style={{ width: "100%", height: "100%", overflow: "hidden" }} dangerouslySetInnerHTML={{ __html: layer.content || "" }} />
      )}

      {/* Resize handles */}
      {isSelected && !layer.locked && !isPlaying && !isEditingText && (
        <>
          {[
            ...
            <div key={h.corner} onMouseDown={e => onResizeStart(e, h.corner)}
              style={{ position: "absolute", width: 8, height: 8, background: "#00d4ff", border: "2px solid #0a0b0f", borderRadius: 2, cursor: h.cursor, ...h.style, zIndex: 10 }} />
          ))}
          {[
            ...
            <div key={h.corner} onMouseDown={e => onResizeStart(e, h.corner)}
              style={{ position: "absolute", width: 6, height: 6, background: "#00d4ff", border: "1.5px solid #0a0b0f", borderRadius: 1, cursor: h.cursor, ...h.style as React.CSSProperties, zIndex: 10 }} />
          ))}
        </>
      )}

      {/* Double-click hint for text layers */}
      {isSelected && layer.type === "text" && !isPlaying && !isEditingText && (
        <div style={{ position: "absolute", bottom: -18, left: 0, fontSize: 9, color: "#00d4ff99", whiteSpace: "nowrap", pointerEvents: "none", userSelect: "none" }}>
...
function CanvasRuler({ width, height, scale, show }: { width: number; height: number; scale: number; show: boolean }) {
  ...
  return (
    <>
      <div style={{ position: "absolute", top: 0, left: RULER_SIZE, right: 0, height: RULER_SIZE, background: "#111218", borderBottom: "1px solid #252736", overflow: "hidden", zIndex: 5, pointerEvents: "none" }}>
        {hMarks.map(x => (
      ...
      <div style={{ position: "absolute", top: RULER_SIZE, left: 0, width: RULER_SIZE, bottom: 0, background: "#111218", borderRight: "1px solid #252736", overflow: "hidden", zIndex: 5, pointerEvents: "none" }}>
        {vMarks.map(y => (
...
export default function Canvas() {
  const { project, selectedLayerId, selectLayer, isPlaying, showGrid, showRulers, updateLayer } = useEditorStore();
  ...
  const handleLayerDoubleClick = useCallback((e: React.MouseEvent, layer: Layer) => {
  ...
  const bgStyle: React.CSSProperties = {
    background: project.background.startsWith("linear") || project.background.startsWith("radial") ? project.background : undefined,
    backgroundColor: !project.background.startsWith("linear") && !project.background.startsWith("radial") && project.background !== "transparent" ? project.background : undefined,
  };
  ...
  return (
    ...
    >
      {showRulers && <CanvasRuler width={project.width} height={project.height} scale={canvasTransform.scale} show={showRulers} />}
        ...
        <div style={canvasStyle}>
          {sortedLayers.map(layer => (
            <LayerRenderer
              ...
              isPlaying={isPlaying}
              isEditingText={layer.id === editingLayerId}
              onMouseDown={e => {
                if (layer.id === editingLayerId) return; // don't drag while editing
                startDrag(e, layer);
              }}
      ...
      <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex items-center gap-2.5 text-[10px] text-studio-subtle pointer-events-none select-none bg-studio-panel/80 backdrop-blur px-3 py-1.5 rounded-full border border-studio-border/50">
        ...
        {editingLayerId && <><span className="text-studio-border">·</span><span className="text-blue-400">Editing Text</span></>}
      </div>
